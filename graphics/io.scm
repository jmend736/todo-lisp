...
; (ass->dot:elem '(a (a b c)))


(define (ass->dot input)
  (let lp ((i 0)
           (str "digraph G {")
           (l input))
    (cond ((> (length l) 0)
           (lp (+ i 1) (string-append str (ass->dot:elem (car l))) (cdr l)))
...
(define (assl->graph assl filename)
  (let ((file-port (open-output-file (string-append filename ".dot"))))
    (write-string (ass->dot assl) file-port)
    (flush-output file-port)
    (let ((port (open-output-string)))
